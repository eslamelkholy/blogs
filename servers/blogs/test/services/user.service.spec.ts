...
describe("Service/UserService", () => {
  ...
  describe("deleteUser", () => {
    it("throws error if user not exists", async () => {
      jest.spyOn(userRepository, "getUserByEmail").mockImplementation(() => null);

      expect(userService.getUserByEmail("email")).rejects.toThrowError(CustomError);
    });

    it("Delete User if User Exists", async () => {
      jest.spyOn(userRepository, "getUserByEmail").mockImplementation(() => Promise.resolve(usersList[0]));
  ...
  describe("createUser", () => {
    ...
    it("Throws User if user Exists ", async () => {
      ...
      expect(userService.createUser(usersList[0] as CreateUserDto)).rejects.toThrowError(CustomError);
    });
  });
  ...
  describe("getUserById", () => {
    ...
    it("Returns user email if Email is Exists", async () => {
      jest.spyOn(userRepository, "getUserById").mockImplementation(() => Promise.resolve(usersList[0]));
  ...
  describe("userSearch", () => {
    it("Returns user email if Email is Exists", async () => {
      jest.spyOn(userRepository, "userSearch").mockImplementation(() => Promise.resolve(mockUserResponse));
